...
const OrderDetail: React.FC<DetailProps> = (props) => {
  ...
  return (
    <PageContainer
      ...
      footer={(detail.orderStatus === 'PART_PAY' || detail.orderStatus === 'WAIT_PAY') ? [
        <Popconfirm
          title="确认取消订单吗?"
          onConfirm={() => handleCancelOrder(detail.id)}
          okText="是"
          cancelText="否"
          key="4"
        >
        ...
        detail.orderStatus === 'PART_PAY' && <Button key="2" type="primary" onClick={() => setIsShow(true)}>线下打款</Button>,
        detail.orderStatus === 'WAIT_PAY' && <Button key="3" type="primary" onClick={() => history.push(`/mall/cart/paying?orderId=${detail.id}`)}>支付</Button>
      ] : []}
    >
      <div>
